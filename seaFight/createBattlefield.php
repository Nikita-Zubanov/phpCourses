...
{
	function writeToFile($field, $idPlayer) 
	{
		$filename = $idPlayer;
	...
	function getShipsLocations($field)
	{
	...
	function getErrorShipPositioning ($field)
	{
		foreach ($field as $rowKey => $row) {
			foreach ($row as $symbolKey => $symbol) {
	...
	function getErrorShipsSize($field)
	{		
		foreach ($field as $rowKey => $row) {
			foreach ($row as $symbolKey => $symbol) {
				$counter = 0;
	...
	function setLoginPlayer($idPlayer)
	{
	...
	function getFieldPlayer() 
	{
		...
		var_dump($this->getShipsLocations($this->fieldPlayers[$idPlayer]));
